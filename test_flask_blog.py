...
class TestFlaskBlog(unittest.TestCase):
    ...
    def setUp(self):
        self.db_fd, self.db_path = tempfile.mkstemp(prefix="test_",suffix=".log") # with文でtempfileを開くのもokかも
        self.app = create_app({
            'TESTING': True,
    ...
    def tearDown(self):
        ...
        # del self.app              ***
        # os.remove(self.db_path)   *** error out: some process catch tmpfile, so cannot remove it.
